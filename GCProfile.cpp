...
//-----v1.86 07-mar-2025	//fixed a compile with -O3 optimiser crash at runtime in prj 'rtl_scan' (did not happen with -O0 compile), by increasing -
...
//-----v1.87 02-jul-2026	//added further number ranges such a 'quecto' and clearing of 'sunits' if numbers are out of coded range, for functions: 'make_engineering_str()'  'make_engineering_str_exp()' 
//-----v1.88 18-aug-2026	//added 'GCProfile::check_get_success()' to check if last GetPrivateProfileXXXX() call 'section/key' was read successfully and no default value was substituted 
							//added 'Index_from_string()' to allow using a string containing ini formatted entries instead of a file
...
//NOTE DOES NOT handle numbers  < e-33  and  > e33 correctly
...
if( dvalue < 1e-1 )
	...
	if( ( dvalue >= 1e-21 ) && ( dvalue < 1e-18 ) )						//v1.87
		{
		dvalue /= 1e-21;
		strpf( sunits, "z" );			//zepto
		goto done;
		}

	if( ( dvalue >= 1e-24 ) && ( dvalue < 1e-21 ) )						//v1.87
		{
		dvalue /= 1e-24;
		strpf( sunits, "y" );			//yocto
		goto done;
		}

	if( ( dvalue >= 1e-27 ) && ( dvalue < 1e-24 ) )						//v1.87
		{
		dvalue /= 1e-27;
		strpf( sunits, "r" );			//ronto
		goto done;
		}

	if( ( dvalue >= 1e-30 ) && ( dvalue < 1e-27 ) )						//v1.87
		{
		dvalue /= 1e-30;
		strpf( sunits, "q" );			//quecto
		goto done;
		}
	...
	if( ( dvalue >= 1e21 ) && ( dvalue < 1e24 ) )						//v1.87
		{
		dvalue /= 1e21;
		strpf( sunits, "Z" );
		goto done;
	...
	if( ( dvalue >= 1e24 ) && ( dvalue < 1e27 ) )						//v1.87
		{
		dvalue /= 1e24;
		strpf( sunits, "Y" );
		goto done;
	...
	if( ( dvalue >= 1e27 ) && ( dvalue < 1e30 ) )						//v1.87
		{
		dvalue /= 1e27;
		strpf( sunits, "R" );
		goto done;
	...
	if( ( dvalue >= 1e30 ) && ( dvalue < 1e33 ) )						//v1.87
		{
		dvalue /= 1e30;
		strpf( sunits, "Q" );
		goto done;
...
if( dvalue < 1e-1 )
	...
	if( ( dvalue >= 1e-21 ) && ( dvalue < 1e-24 ) )						//v1.87
		{
		dvalue /= 1e-21;
		strpf( sunits, "e-21" );			//zeppo
		goto done;
		}

	if( ( dvalue >= 1e-24 ) && ( dvalue < 1e-27 ) )						//v1.87
		{
		dvalue /= 1e-24;
		strpf( sunits, "e-24" );			//yocto
		goto done;
		}

	if( ( dvalue >= 1e-27 ) && ( dvalue < 1e-30 ) )						//v1.87
		{
		dvalue /= 1e-27;
		strpf( sunits, "e-27" );			//ronto
		goto done;
		}

	if( ( dvalue >= 1e-30 ) && ( dvalue < 1e-33 ) )						//v1.87
		{
		dvalue /= 1e-30;
		strpf( sunits, "e-30" );			//quecto
		goto done;
		}
	...
	if( ( dvalue >= 1e21 ) && ( dvalue < 1e24 ) )						//v1.87
		{
		dvalue /= 1e21;
		strpf( sunits, "e%s21", splus.c_str() );
		goto done;
		}
	...
	if( ( dvalue >= 1e24 ) && ( dvalue < 1e27 ) )						//v1.87
		{
		dvalue /= 1e24;
		strpf( sunits, "e%s24", splus.c_str() );
		goto done;
		}
	...
	if( ( dvalue >= 1e27 ) && ( dvalue < 1e30 ) )						//v1.87
		{
		dvalue /= 1e27;
		strpf( sunits, "e%s27", splus.c_str() );
		goto done;
		}
	...
	if( ( dvalue >= 1e30 ) && ( dvalue < 1e33 ) )						//v1.87
		{
		dvalue /= 1e30;
		strpf( sunits, "e%s30", splus.c_str() );
		goto done;
		}
...
Index();		//see also 'Index_from_string()'
}
...
//read 'ss' into csAll str, this allow a user to specifiy an string that has a ini formatted data (instead of a ini file)
//see ALSO 'Index()'
int GCProfile::Index_from_string( string &ss )							//v1.88
{
string s1;
mystr m1;
...
ss += "\n";																//ensure ending LF so 'gets()' call below gets the last line of non white text
m1 = ss;

csAll="\n";	//make sure first line is a blank so first [Section] has a LF before it, helps simplify code for searching for a Section

//printf( " ----------- GCProfile::Index_from_string() ----------\n" );
...
//		fgets(szStr,sizeof(szStr),fp);
		if( !m1.gets( s1 ) ) 
			{
//			printf( "CProfile::Index_from_string() end reached\n" );
			break;
			}
...
//			}
		...
		csAll += csLine;
		csAll += "\n";
//printf( "CProfile::Index_from_string() csLine '%s'\n", csLine.c_str() );
		}
	}
	
//printf( "%s", csAll.c_str()  );
//printf( " -----------------------------------------------------\n" );
...
//NOTE this is not valid if string formatted ini was specified in place of a file loaded ini, refer 'Index_from_string()'
...
bool GCProfile::check_get_success()										//v1.88
{
return last_op_success;
}
