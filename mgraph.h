...
mg_col_tag col_graticle_center;											//v1.38
mg_col_tag col_graticle;												//v1.38
...
bool b_take_focus_on_enter;			//v1.38
...
bool get_plot_values_zero_offsets( int trc, double &x1, double &y1, double &x2, double &y2, double &x3, double &y3, double &x4, double &y4, bool btrace_offs_zero, bool bplot_offs_zero ); //v1.37
bool get_plot_value_coord_zero_offsets( int trc, double &x1, double &y1, bool btrace_offs_zero, bool bplot_offs_zero ); //1.38
...
bool get_pixel_position_as_trc_values( int trc, int px, int py, double &valx, double &valy, bool btrace_offs_zero, bool bplot_offs_zero );	//v1.35
...
void draw_circle_double( double d_x, double d_y, int iw, int ih, int rr, int gg, int bb );
...
mg_col_tag col_delta_text_multi_1[ cn_fast_mgraph_cnt_max ];				//v1.38  refer 'col_delta_text[]'
mg_col_tag col_delta_text_multi_2[ cn_fast_mgraph_cnt_max ];
mg_col_tag col_delta_text_multi_3[ cn_fast_mgraph_cnt_max ];
mg_col_tag col_delta_text_multi_4[ cn_fast_mgraph_cnt_max ];
...
mg_col_tag col_delta_text[cn_fast_mgraph_cnt_max];				//v1.38, used for selected sample text colour, refer 'col_delta_text_multi_1[]'
...
double fit_plot_min_maxy[ cn_fast_mgraph_cnt_max ];				//v1.39, user specified min/max from fit 'fit_plot_to_this_y_range()'
...
bool show_tick_guide_lines_x[ cn_fast_mgraph_cnt_max ];					//v1.38, see also 'col_tick_guide_lines_x[]'
bool show_tick_guide_lines_y[ cn_fast_mgraph_cnt_max ];

bool b_show_sel_dB_y;													//show dB value of sel sample's y value
bool b_show_zero_x_axis;												//v1.38, shows a tick where x-axis is zero, if it's visible
...
mg_col_tag col_tick_guide_lines_x[ cn_fast_mgraph_cnt_max ];		//v1.38, refer 'show_tick_guide_lines_x[]'

mg_col_tag col_tick_guide_lines_y[ cn_fast_mgraph_cnt_max ];		//v1.38, refer 'show_tick_guide_lines_y[]'
...
void set_col_delta_text1( int gph_idx, const char *col_axis );			//v1.38
void set_col_delta_text2( int gph_idx, const char *col_axis );
void set_col_delta_text3( int gph_idx, const char *col_axis );
void set_col_delta_text4( int gph_idx, const char *col_axis );

void set_col_delta_text1_rgb( int gph_idx, uint8_t rr, uint8_t gg, uint8_t bb );	//v1.38
void set_col_delta_text2_rgb( int gph_idx, uint8_t rr, uint8_t gg, uint8_t bb );
void set_col_delta_text3_rgb( int gph_idx, uint8_t rr, uint8_t gg, uint8_t bb );
void set_col_delta_text4_rgb( int gph_idx, uint8_t rr, uint8_t gg, uint8_t bb );
...
double fit_plot_get_extremey( unsigned int grph_idx );							//1.39
...
//uses doubles for theta inc, this helps if freq0 is high
template< typename T >
void make_cos_sin( unsigned int srate, double freq0, double ampl_cos, double ampl_sin, double dc_offset_cos, double dc_offset_sin, unsigned int cnt, std::vector<T> &vcos, std::vector<T> &vsin )
{
vcos.clear();
vcos.reserve( cnt );
vsin.clear();
...
for( int i = 0; i < cnt; i++ )
	{
	double f0 = dc_offset_cos + ampl_cos * cos( theta0 );
	vcos.push_back( (T)f0 );


	f0 = dc_offset_sin + ampl_sin * sin( theta0 );
	vsin.push_back( (T)f0 );
...
//analyse an fir response between 2 freqs
template< typename T >
void freqz_fir_between_freqs(
    vector<T> &vcoeff,
    unsigned int num_points,
    unsigned int srate,
...
)
{
    ...
    vfreq_out.resize(num_points);
    vamp_out.resize(num_points);

    for(unsigned int k = 0; k < num_points; k++)
    {


        float freq = (float)freq_start +
                 ((float)(freq_stop - freq_start) * (float)k) /
                 (float)(num_points - 1);

       float omega = (float)twopi * freq / (float)srate;
...
//        vomega[k] = omega;
        ...
        vamp_out[k] = (T)sqrt((double)(re*re + im*im));
...
//-------------------------------------------------------------------------------
// analyse an iir response at a single frequency
template<
    typename Tcoef,
    typename Tfreq,
    typename Tfs,
    typename Tout
>
int freqz_sos_inline_single_freq(
    vector<Tcoef> &vnumer,   // [b0,b1,b2, b0,b1,b2, ...]
    vector<Tcoef> &vdenom,   // [1,a1,a2, 1,a1,a2, ...]
    Tfs fsrate,              // sample rate in Hz
    Tfreq freq_in,           // frequency in Hz
    Tout &amp_out
)
{
    ...
    const unsigned int nsec =
        (unsigned int)(vnumer.size() / 3);

    typedef double Tcalc;

    // convert Hz -> rad/sample
    Tcalc omega =
        (Tcalc)twopi *
        (Tcalc)freq_in /
        (Tcalc)fsrate;
    ...
    for(unsigned int sec = 0; sec < nsec; sec++)
    {

        unsigned int idx = sec * 3;

        Tcalc b0 = (Tcalc)vnumer[idx + 0];
        Tcalc b1 = (Tcalc)vnumer[idx + 1];
        Tcalc b2 = (Tcalc)vnumer[idx + 2];

        Tcalc a0 = (Tcalc)vdenom[idx + 0];
        Tcalc a1 = (Tcalc)vdenom[idx + 1];
        Tcalc a2 = (Tcalc)vdenom[idx + 2];
        ...
        Tcalc ang1 = -omega;
        Tcalc ang2 = -2.0 * omega;

        Tcalc c1 = cos((double)ang1);
        Tcalc s1 = sin((double)ang1);

        Tcalc c2 = cos((double)ang2);
        Tcalc s2 = sin((double)ang2);
        ...
        Tcalc re_sec =
            (re_b*re_a + im_b*im_a) / denom_mag2;

        Tcalc im_sec =
            (im_b*re_a - re_b*im_a) / denom_mag2;

        Tcalc re_tmp =
            re_h*re_sec - im_h*im_sec;

        Tcalc im_tmp =
            re_h*im_sec + im_h*re_sec;
