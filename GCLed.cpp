/*
Copyright (C) 2007-2026 BrerDawg
...
//---- v1.06    2020-sep-19     //changed 'if ( e & FL_ENTER )' to 'if ( e == FL_ENTER )' as this was stopping a parent 'FL_Scroll' scrolling via mousewheel when mouse inside 'GCLed'
...
//---- v1.12    2026-apr-13		//moded various functions to not accept negative indexes
								//added 'override_col()'
...
if( ioverride_r || ioverride_g || ioverride_b )							//any override col spec
	{
	rr = ioverride_r;	
	gg = ioverride_g;	
	bb = ioverride_b;	
...
if ( e == FL_ENTER )		//v1.06, was 'e & FL_ENTER'
    {
 	left_button = 0;		//v1.12, this ensured if callback causes led to lose focus and miss 'FL_RELEASE', these are returned to a known state on mouse entering led wdg again
	middle_button = 0;
	right_button = 0;
...
int GCLed::SetColIndex( int uiIndex,int iRin,int iGin,int iBin)
{
if( uiIndex >= cnMaxColIndex ) return 0;
if( uiIndex < 0 ) return 0;												//v1.12
...
void GCLed::set_index_col_rgb( int idx, uint8_t rr, uint8_t gg, uint8_t bb  )	//v1.13
{

if( idx >= cnMaxColIndex ) return;
if( idx < 0 ) return;													//v1.12
...
//forces led to have a specific colour regardless of its iIndex or brightness settings
//set all cols to zero to return to normal iIndex cols
void GCLed::override_col( int rr, int gg, int bb )	//v1.12
{
ioverride_r = rr;
ioverride_g = gg;
ioverride_b = bb;
