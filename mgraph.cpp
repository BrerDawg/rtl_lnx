/*
Copyright (C) 2022 BrerDawg, et. al.
...
//v1.35				12-apr-2026			//moded 'get_pixel_position_as_trc_values()' to allow caller to zero 'trace_offy' and 'plot_offs' effect on 'valx, 'valy'
//v1.36				21-apr-2026			//added 'draw_circle_double()', have adj 'b_show_hov_nearest_sample' draw code to use this, needed extra precision when x-axis holds GHz values for example
//v1.37				28-apr-2026			//added 'get_plot_values_zero_offsets()' to allow caller to zero 'trace_offy' and 'plot_offs' effect on plot vals returned 
										//made mgraph 'inside_control' public
										//added 'dsp_utils_code::make_cos_sin()'

//v1.38				07-may-2026			//added 'get_plot_value_coord_zero_offsets()' 'show_tick_guide_lines_x[]' 'col_tick_guide_lines_x[]' etc
										//added to 'fast_mgraph'  'col_delta_text[]'  'col_delta_text_multi_1[]'  'set_col_delta_text1()'  'set_col_delta_text1_rgb()'  etc
										//added to 'fast_mgraph'  'b_show_zero_x_axis'
										//moded 'fast_mgraph' to move x-axis values to bottom and be shown with human friendly stepping, also added x-axis vertical guide lines
										//added to 'mgraph'  'col_graticle_center'  'col_graticle'
										//added 'dsp_utils_code::freqz_sos_inline_single_freq()'
										//added to 'mgraph'  'b_take_focus_on_enter'
						
						
//v1.39				13-jul-2026			//added to 'fast_mgraph' 'fit_plot_get_extremey()'
										//fixed 'm' mousewheel y scale adj feature, refer 'if( p->keym[ ii ] )'
...
col_graticle_center.r = 150;											//v1.38
col_graticle_center.g = 150;
col_graticle_center.b = 150;

col_graticle.r = 90;													//v1.38
col_graticle.g = 90;
col_graticle.b = 90;
...
//see also 'get_plot_values_zero_offsets()'
...
//takes coord values relative to spec trace, and converts them to nearest integer plot values
//set flags to zero offsets as required
//SEE ALSO 'get_plot_value_coord_zero_offsets()'
bool mgraph::get_plot_values_zero_offsets( int trc, double &x1, double &y1, double &x2, double &y2, double &x3, double &y3, double &x4, double &y4, bool btrace_offs_zero, bool bplot_offs_zero )
{
if ( trce.size() == 0 ) return 0;
...
//v1.38
//takes 1 coord value relative to spec trace, and converts it to nearest integer plot values
//set flags to zero offsets as required
//SEE ALSO 'get_plot_values_zero_offsets()'
bool mgraph::get_plot_value_coord_zero_offsets( int trc, double &x1, double &y1, bool btrace_offs_zero, bool bplot_offs_zero )
{
if ( trce.size() == 0 ) return 0;
...
//					}
            ...
            case en_dobt_rect:
				...
				fl_begin_line();
					...
					fl_vertex( ix, iyy );
					fl_vertex( ix, iy );
//printf( "mgraph::draw_user_objs() - en_dobt_rect  xx1: %f, yy1: %f  xx2: %f, yy2: %f\n", xx1, yy1, xx2, yy2 );
				fl_end_line();
            break;
...
//this is called by 'draw()', use it by setting 'dbg_type' 'idbg0...' etc, DON'T CALL this directly, instead use 'draw_circle_float()'
//''fx' and 'fy' must be in the 'trace_tag.pnt' coord system
void mgraph::draw_circle_double( double d_x, double d_y, int iw, int ih, int rr, int gg, int bb )		//v1.36
{
int trc = 0;
...
//	fl_color( 150, 150, 150 );
	fl_color( col_graticle_center.r, col_graticle_center.g, col_graticle_center.b );	//v1.38
    fl_line_style ( FL_SOLID, 1 );         					//for windows you must set line setting after the colour, see the manual
...
//	fl_color( 90, 90, 90 );
	fl_color( col_graticle.r, col_graticle.g, col_graticle.b );			//v1.38
    fl_line_style ( FL_SOLID, 1 );         					//for windows you must set line setting after the colour, see the manual
	...
	if( hov_nearest_sample_trc < trce.size() )
		...
		if( ( hov_nearest_sample_idx >= 0 ) && ( hov_nearest_sample_idx < trce[hov_nearest_sample_trc].pnt.size() ) )
			...
			for( int i = 0; i < vhov_nearest_sample_idx.size(); i++ )
				...
				double d_x = trce[hov_nearest_sample_trc].pnt[idx].x;
				double d_y = trce[hov_nearest_sample_trc].pnt[idx].y;
				...
				draw_circle_double( d_x, d_y, hov_nearest_sample_circle_radiusx, hov_nearest_sample_circle_radiusy, hov_nearest_col.r, hov_nearest_col.g, hov_nearest_col.b );	//v1.36	
				}
			}
		fl_line_style( FL_SOLID, 1 );        							//for windows you must set line setting after the colour, see the manual
		}
...
//these have 'woffx + bkgd_border_left'  and ' woffy + bkgd_border_top'  subtracted
...
if( !get_pixel_position_as_trc_values( trc, px, py, mx, my, 1, 1 ) ) return 0;	//v1.35
...
bool mgraph::get_pixel_position_as_trc_values( int trc, int px, int py, double &valx, double &valy, bool btrace_offs_zero, bool bplot_offs_zero )		//v1.29, v1.35
{
if ( trce.size() == 0 ) return 0;
...
//printf("mgraph::hov_nearest_sample_build() - mousex_low_sample_idx %d, hov_nearest_sample_range %d\n", trce[ trc ].mousex_low_sample_idx, hov_nearest_sample_range );
...
for( int i = 0; i < ( hov_nearest_sample_range * 2 ) + 1; i++ )			//collect two times the range + 1
	{
//printf("mgraph::hov_nearest_sample_build() - %02d: mousex_low_sample_idx %d, idx %d \n", i,  trce[ trc ].mousex_low_sample_idx, idx );
...
if ( e == FL_ENTER )	
	...
	if( b_take_focus_on_enter ) take_focus();							//v1.38
...
//this returns the current y scale min or max
double fast_mgraph::fit_plot_get_extremey( unsigned int grph_idx )		//v1.39
{
	
if( grph_idx >= cn_fast_mgraph_cnt_max )
	{
	grph_idx = 0;
	}

return fit_plot_min_maxy[grph_idx];
}
...
if(vb)printf("fast_mgraph::fit_plot() - dy %f %f\n", dy, middley );
	...
	fit_plot_min_maxy[ ii ] = extremey;									//1.39
...
//	gph[ii]->get_trace_min_max( 0, xmin, xmax, ymin, ymax );
	...
	fit_plot_min_maxy[ii] = extremey;									//v1.39
...
if( p->keym[ ii ] )			//max deflection y
	{
	double extrme_y = p->fit_plot_get_extremey( ii );					//v1.39

	if( int0 > 0.0 ) extrme_y *= 1/1.25;
	if( int0 < 0.0 ) extrme_y *= 1.25;
	p->fit_plot_to_this_y_range( ii, -extrme_y, extrme_y );				//v1.39


//printf("cb_fast_gph_mousewheel() - extrme_y %f\n", extrme_y );
/*	
	float factor = 1;
...
if( p->keya[ ii ] )			//amplitude adj
	{
//printf("cb_fast_gph_mousewheel() sel_trc: %d\n",sel_trc);
...
if( gph_cnt == 1 )										//one graph, multi trace?
	...
	col_delta_text_multi_1[0].r = 300;							//v1.38
	col_delta_text_multi_1[0].g = 0;
	col_delta_text_multi_1[0].b = 100;

	col_delta_text_multi_2[0].r = 300;
	col_delta_text_multi_2[0].g = 0;
	col_delta_text_multi_2[0].b = 100;

	col_delta_text_multi_3[0].r = 300;
	col_delta_text_multi_3[0].g = 0;
	col_delta_text_multi_3[0].b = 100;

	col_delta_text_multi_4[0].r = 300;
	col_delta_text_multi_4[0].g = 0;
	col_delta_text_multi_4[0].b = 100;
	...
	col_tick_guide_lines_x[0].r = 230;
	col_tick_guide_lines_x[0].g = 230;
	col_tick_guide_lines_x[0].b = 230;

	col_tick_guide_lines_y[0].r = 230;
	col_tick_guide_lines_y[0].g = 230;
	col_tick_guide_lines_y[0].b = 230;
...
for( int i = 0; i < gph_cnt; i++ )
	...
	if( gph_cnt >= 2 )							//if multi graph, set suitable details
		...
		col_delta_text_multi_1[i].r = 300;								//v1.38
		col_delta_text_multi_1[i].g = 0;
		col_delta_text_multi_1[i].b = 100;

		xtick_marks[i] = 16;
		ytick_marks[i] = 20;

		col_tick_guide_lines_x[i].r = 230;
		col_tick_guide_lines_x[i].g = 230;
		col_tick_guide_lines_x[i].b = 230;

		col_tick_guide_lines_y[i].r = 230;
		col_tick_guide_lines_y[i].g = 230;
		col_tick_guide_lines_y[i].b = 230;
	...
	*/
	...
	show_tick_guide_lines_x[i] = 1;
	show_tick_guide_lines_y[i] = 1;
...
if(1)
	...
	gph[ ii ]->get_pixel_position_as_trc_values( 0, imx, imy, valx, valy, 0, 0 );	//v1.35
//	dmx = valx;
	}
...
//	strpf( s1, "dx:%.2f dy:%.2f (Freq:%.2f Hz)", dx, dy, 1.0/dx );
	...
	mdo.y1 = 9;
	mdo.stext = s1;
	mdo.r = col_delta_text[ii].r;
	mdo.g = col_delta_text[ii].g;
	mdo.b = col_delta_text[ii].b;
	mdo.line_style = (en_mgraph_line_style)FL_SOLID;
...
// ----- scale dx2 within range 1.0-->10.0, provides human friendly quantizing of x-axis tick guide line values ------ v1.38
// below code avoids helps avoid bunching/overlapping x-axis tick values

double dx = (maxx/scalex - minx/scalex );
double dx2 = dx;
double dx3 = 1.0;
...
if( dx2 == 0.0 )
	{
	printf( "fast_mgraph::update_fg_user_obj() - 'dx' is %f, cannot make it sit within range  1.0-->10.0, x-axis tick values will not be correct\n", dx );
	}
else{
	while( dx2 != 0.0 )
		{
		...
		if( jj > 10000 )
			{
			printf( "fast_mgraph::update_fg_user_obj() - failed after %d loops: to scale 'dx' %f to sit within range  1.0-->10.0, x-axis tick values will not be correct\n", jj, dx );
			break;
			}
		if( dx2 < 1.0 )
			{
	...
	dx3 = 0.1;
	if( dx2 > 1.5 ) dx3 = 0.2;						//provide human friendly quantizing of x-axis tick guide line values
	if( dx2 > 3.1 ) dx3 = 0.25;
	if( dx2 > 3.8 ) dx3 = 0.25;
	if( dx2 > 4.0 ) dx3 = 0.5;
	if( dx2 > 8.0 ) dx3 = 0.75;
...
double dt = dx3 / factor_dx;

//printf( "fast_mgraph::update_fg_user_obj() ddddddddddd dx %f  dx2 %f  factor_dx %f  dx3 %f   dt %f\n", dx, dx2, factor_dx, dx3, dt );
//----------------------------------------------
...
// ----- scale minx2 within range 1.0-->10.0, provide human friendly quantizing of x-axis starting value for tick guide lines ------ 
double minx3 = 0.0;

{
double minx2 = minx;

double factor_minx = 1;
double iptr = 0;

jj = 0;
if( minx2 == 0.0 )														//already rounded?
	{
	}
else{
	
	bool b_positive = 1;
	if( minx2 < 0.0 ) b_positive = 0;
	
	minx2 = modf( minx2, &iptr );										//seperate fraction, also keep integer for below
	...
	while( minx2 != 0.0 )
		...
		if( jj > 10000 )
			{
			printf( "fast_mgraph::update_fg_user_obj() - failed after %d loops: to scale 'minx' %f to sit within range  1.0-->10.0, x-axis tick start value will not be well rounded\n", jj, minx2 );
			break;
			}
	...
	if( b_positive )
		{
//		if( minx2 > 0.0 ) minx3 = 0.0;									//provide human friendly starting value of x-axis tick guide lines
		if( minx2 > 2.5 ) minx3 = 0.0;									//provide human friendly starting value of x-axis tick guide lines
		if( minx2 > 5.0 ) minx3 = 2.5;
		if( minx2 > 7.5 ) minx3 = 5.0;
		}
	else{
		minx2 = -minx2;													//restore sign

		minx3 = minx2;//nearbyint( minx2 );
//		if( minx2 < 0.0 ) minx3 = -100.0;

//		if( minx2 < 0.0 ) minx3 = -1.15;
//		if( minx2 < -1.15 ) minx3 = -1.2;
//		if( minx2 < -1.2 ) minx3 = -1.25;
//		if( minx2 < -1.25 ) minx3 = -1.5;
//		if( minx2 < -1.5 ) minx3 = -2.0;

//		if( minx2 < 0.0 ) minx3 = -10.0;
//		if( minx2 < -2.5 ) minx3 = -10.0;
//		if( minx2 < -5.0 ) minx3 = -10.0;
//		if( minx2 < -7.5 ) minx3 = -10.0;
//printf( "fast_mgraph::update_fg_user_obj() - zzzzzzzzzzzzzznegative minx2 %f  minx3 %f\n", minx2, minx3 );
		}
	}


minx3 = minx3 / factor_minx;

minx3 += iptr;															//recombine integer part

minx3 = -0.01615;
minx3 = -100.0133;
//printf( "fast_mgraph::update_fg_user_obj() ddddddddddd minx %f  minx2 %f  factor_minx %f  minx3 %f\n", minx, minx2, factor_minx, minx3 );
...
//----- quantize 'minx' to keep x tick guide line values human friendly ------- v1.38
double minx3 = minx;
...
bool btens_is_fract = 0;
if( itens < 0 ) btens_is_fract = 1;

itens = fabsf( itens );

double factor_dt = (int) powf( 10, itens);

double minx4;
double minx5; 
if( btens_is_fract )
	{
	minx4 = minx3 * factor_dt;											//quantize digits right of decimal point (fractional wise)

	minx5 = nearbyint( minx4 );
	minx5 = minx5 / factor_dt;
	}
else{
	minx4 = minx3 / factor_dt;											//quantize digits left of decimal point (integer wise)

	minx5 = nearbyint( minx4 );
	minx5 = minx5 * factor_dt;
	}

//if(id0 == 2)printf( "fast_mgraph::update_fg_user_obj() - zzzzzzzzzzzzzz dt %f  factor_dt %f  minx3 %f  minx4 %f  minx5 %f\n", dt, factor_dt, minx3, minx4, minx5 );
...
//for( int i = 0; i < tick; i++ )
for( int i = 0; i < 100000; i++ )											//loop over a larger number than req, 'break' below will halt loop
	{
...
//printf( "fast_mgraph::update_fg_user_obj() minx %f %f  posx %f   mdo.x1 %f\n", minx, minx, posx, mdo.x1);
...
//-------- add a zero tick where possible -------

	if( b_show_zero_x_axis )
		{
		if( i == 0 )														//do a once off x-axis = 0.0 tick
			{
			if( !b_x_axis_values_derived[ii] )								//does user NOT want to display specific x axis values ? (only works for plots without a supplied x vector)
				{
				mdo.x1 = 0.0;
				double x0, y0;
				...
				gph[ii]->get_plot_value_coord_zero_offsets( 0, x0, y0, 1, 1 );		//get tick position in pixels


				bool bshow_zero = 1;		
				if( x0 < -50 )														//past left edge? 			
					{
					bshow_zero = 0;
					}

				if( x0 > gph[ii]->w() + 50 )										//past right edge? 			
					{
					bshow_zero = 0;
					}
				...
				//	mdo.y1 = miny + (maxy - miny)/2 - (maxy - miny) * 0.025/fscly_tickx;	//v1.22;
					...
					mdo.r = col_tick_guide_lines_x[ii].r;
					mdo.g = col_tick_guide_lines_x[ii].g;
					mdo.b = col_tick_guide_lines_x[ii].b;
...
//	if(id0 == 2)printf( "fast_mgraph::update_fg_user_obj() - ii %d  minx5 %f\n", ii, minx5 );
...
//mdo.x1 = -100.0 + i * dt;
	...
	if( x0 > gph[ii]->w() + 50 )										//past right edge? 			
		{
//		if(id0 == 2)printf("fast_mgraph::update_fg_user_obj() iiiiiiiiiii  %d\n", i );
		break;
		}
...
//	printf( "fast_mgraph::update_fg_user_obj() x0 %f %f\n", mdo.x1, x0 );
...
//sig_dig = 5;
	...
	m1.make_engineering_str( snum, sunits, scombined, sig_dig, valx, sappend_num, sappend_units );		//v1.38
	mdo.type = en_dobt_text;
	...
	mdo.draw_ordering = 2;				//draw before graticle, 0 = before grid,  1 = after graticle but before traces,   2 = after traces
//	strpf( s1, "%s", scombined.c_str() );
	strpf( s1, "%s%s", snum.c_str(), sunits.c_str() );
	mdo.arc1 = 0;
	mdo.arc2 = 0;
	mdo.stext = s1;
...
//	mdo.y1 = miny + (maxy - miny)/2 - (maxy - miny) * 0.025/fscly_tickx;	//v1.22;
	...
	//--- x-axis long vert tick guide lines ---
	if( show_tick_guide_lines_x[ii] )
		{
		mdo.type  = en_dobt_line;
		mdo.draw_ordering = 1;				//draw before graticle, 0 = before grid,  1 = after graticle but before traces,   2 = after traces
...
//printf("fast_mgraph::update_fg_user_obj() - shift_trc_y %f shfty %f\n", shift_trc_y[ ii ][0], shfty );
	...
	//	if( mdo.y1 > 11.0f ) break;
		...
		mdo.r = col_tick_guide_lines_y[ii].r;
		mdo.g = col_tick_guide_lines_y[ii].g;
		mdo.b = col_tick_guide_lines_y[ii].b;
...
//------ v1.38
//set 'gph_idx' to zero, unless there are multiple graphs
void fast_mgraph::set_col_delta_text1_rgb( int gph_idx, uint8_t rr, uint8_t gg, uint8_t bb )
{
if ( gph_idx >= gph_cnt ) return;
if ( gph_idx < 0 ) return;
...
col_delta_text_multi_1[gph_idx] = col;
}
...
//set 'gph_idx' to zero, unless there are multiple graphs
void fast_mgraph::set_col_delta_text2_rgb( int gph_idx, uint8_t rr, uint8_t gg, uint8_t bb )
{
if ( gph_idx >= gph_cnt ) return;
if ( gph_idx < 0 ) return;
...
col_delta_text_multi_2[gph_idx] = col;
}
...
//set 'gph_idx' to zero, unless there are multiple graphs
void fast_mgraph::set_col_delta_text3_rgb( int gph_idx, uint8_t rr, uint8_t gg, uint8_t bb )
{
if ( gph_idx >= gph_cnt ) return;
if ( gph_idx < 0 ) return;
...
col_delta_text_multi_3[gph_idx] = col;
}
...
//set 'gph_idx' to zero, unless there are multiple graphs
void fast_mgraph::set_col_delta_text4_rgb( int gph_idx, uint8_t rr, uint8_t gg, uint8_t bb )
{
if ( gph_idx >= gph_cnt ) return;
if ( gph_idx < 0 ) return;
...
col_delta_text_multi_4[gph_idx] = col;
}
//------
...
//------ v1.38
//set 'gph_idx' to zero, unless there are multiple graphs
void fast_mgraph::set_col_delta_text1( int gph_idx, const char *col_axis )
{
if ( gph_idx >= gph_cnt ) return;
if ( gph_idx < 0 ) return;
...
//set 'gph_idx' to zero, unless there are multiple graphs
void fast_mgraph::set_col_delta_text2( int gph_idx, const char *col_axis )
{
if ( gph_idx >= gph_cnt ) return;
if ( gph_idx < 0 ) return;
...
//set 'gph_idx' to zero, unless there are multiple graphs
void fast_mgraph::set_col_delta_text3( int gph_idx, const char *col_axis )
{
if ( gph_idx >= gph_cnt ) return;
if ( gph_idx < 0 ) return;
...
//set 'gph_idx' to zero, unless there are multiple graphs
void fast_mgraph::set_col_delta_text4( int gph_idx, const char *col_axis )
{
if ( gph_idx >= gph_cnt ) return;
if ( gph_idx < 0 ) return;
...
//printf("here 000\n" );
	...
	col_delta_text[0] = col_delta_text_multi_1[0];							//v1.38
	col_delta_text[1] = col_delta_text_multi_2[0];
	col_delta_text[2] = col_delta_text_multi_3[0];
	col_delta_text[3] = col_delta_text_multi_4[0];
...
float goertzel_mag_real_float( vector<float> &vf0, float fs, float f0 )	//v1.40
{
...
//code obtained via chatgpt5
//'vf0' holds sine wfm samples to be measured, >100 sine cycles for accuracy are needed
//'f0' is the freq to be measured, make SURE that this is exact freq the 'vf0' samples were recorded with, ELSE magn will be INCORRECT
//'fs' is the samplerate
//needs more than >100 sine cycles to average out spurious values and focus in on the actual 'f0' to be measured
//returns magnitude of calc'd at freq 'f0'
double goertzel_mag_real_double( vector<double> &vf0, double fs, double f0 )	//v1.40
{
	
	
double w = 2.0f * M_PI * f0 / fs;
double coeff = 2.0f * cosf(w);
